...
HID_API_EXPORT hid_device * HID_API_CALL hid_open_path(const char *path)
{
	...
	res = HidD_GetPreparsedData(dev->device_handle, &dev->pp_data);
	if (!res) {
		register_error(dev, "HidD_GetPreparsedData");
		goto err;
	}
	nt_res = HidP_GetCaps(dev->pp_data, &dev->caps);
	if (nt_res != HIDP_STATUS_SUCCESS) {
		register_error(dev, "HidP_GetCaps");	
		goto err_pp_data;
	}
	dev->input_report_length = dev->caps.InputReportByteLength;
...
err_pp_data:
		HidD_FreePreparsedData(dev->pp_data);
err:	
