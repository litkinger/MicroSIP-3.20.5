...
BEGIN_MESSAGE_MAP(AccountDlg, CDialog)
    ...
    ON_NOTIFY(NM_CLICK, IDC_SYSLINK_AVPF, &AccountDlg::OnNMClickSyslinkAvpf)
...
void AccountDlg::Load(int id)
{
    ...
    ((CButton*)GetDlgItem(IDC_AVPF))->SetCheck(m_Account.avpf);

    //((CButton*)GetDlgItem(IDC_EARLY_MEDIA))->SetCheck(m_Account.early_media);
...
void AccountDlg::OnBnClickedOk()
{
    ...
    m_Account.avpf = ((CButton*)GetDlgItem(IDC_AVPF))->GetCheck();
   // m_Account.ice = ((CButton*)GetDlgItem(IDC_EARLY_MEDIA))->GetCheck();
...
void AccountDlg::OnNMClickSyslinkAvpf(NMHDR *pNMHDR, LRESULT *pResult)
{
    OpenHelp(_T("avpf"));
    *pResult = 0;
}
