...
BEGIN_MESSAGE_MAP(Dialer, CBaseDialog)
    ...
    ON_BN_CLICKED(IDC_DIALER_EM, &Dialer::OnBnClickedEM)
...
void Dialer::RebuildButtons(bool init)
{
    ...
    if (IsChild(&m_ButtonEm)) {
        if (m_ToolTip) {
            m_ToolTip.DelTool(&m_ButtonEm);
        }
        m_ButtonEm.DestroyWindow();
    }
    ...
    bool addEm = true;
	if (addDND || addFWD || addAA || addAC || addConf || addRec || addEm) {
		CRect windowRect;
		if (!init) {
			GetWindowRect(windowRect);
			SetWindowPos(NULL, 0, 0, windowSize.x, windowSize.y, SWP_NOZORDER | SWP_NOMOVE);
        ...
        if (addEm) {
            m_ButtonEm.Create(Translate(_T("EM")), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_AUTOCHECKBOX | BS_PUSHLIKE , rect, this, IDC_DIALER_EM);
            m_ButtonEm.SetFont(GetFont());
            m_ButtonEm.SetCheck(accountSettings.EM ? BST_CHECKED : BST_UNCHECKED);
            AutoMove(m_ButtonEm.m_hWnd, 100, 100, 0, 0);
            if (m_ToolTip) {
                m_ToolTip.AddTool(&m_ButtonEm, Translate(_T("Early Media")));
            }
            rect.left -= stepPx;
            rect.right -= stepPx;
        }
...
void Dialer::OnBnClickedEM()
{
    CButton *button = (CButton*)GetDlgItem(IDC_DIALER_EM);
    accountSettings.EM = button->GetCheck() == BST_CHECKED;
    accountSettings.SettingsSave();
    mainDlg->UpdateWindowText();
}
