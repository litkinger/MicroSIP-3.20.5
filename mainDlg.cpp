	...
	pjsip_rx_data *rdata)
{
	...
	if (busy) {
		...
		if (reject) {
            ...
            user_data->earlyMedia = accountSettings.EM;
...
LRESULT CmainDlg::onIncomingCall(WPARAM wParam, LPARAM lParam)
{
	...
	if (autoAnswer && AutoAnswer(call_info->id)) {
        ...
        if (user_data->earlyMedia) {
            pjsua_call_answer(call_info->id, 183, NULL, NULL);
        } else {
...
void CmainDlg::PJAccountConfig(pjsua_acc_config *acc_cfg, Account *account)
{
    ...
    acc_cfg->rtcp_fb_cfg.dont_use_avpf = account->avpf ? PJ_FALSE : PJ_TRUE;
...
/*
	acc_cfg->turn_cfg_use = accountSettings.enableSTUN ? PJSUA_TURN_CONFIG_USE_CUSTOM : PJSUA_TURN_CONFIG_USE_DEFAULT;
	acc_cfg->turn_cfg.enable_turn =  accountSettings.enableSTUN ? PJ_TRUE : PJ_FALSE;
	acc_cfg->turn_cfg.turn_server = MSIP::StrToPjStr(accountSettings.stun);
	acc_cfg->turn_cfg.turn_conn_type = PJ_TURN_TP_UDP;
	acc_cfg->turn_cfg.turn_auth_cred.type = PJ_STUN_AUTH_CRED_STATIC;
    acc_cfg->turn_cfg.turn_auth_cred.data.static_cred.username = MSIP::StrToPjStr(CString("1618551789:1803"));
    acc_cfg->turn_cfg.turn_auth_cred.data.static_cred.data_type = PJ_STUN_PASSWD_PLAIN;
    acc_cfg->turn_cfg.turn_auth_cred.data.static_cred.data = MSIP::StrToPjStr(CString("01/JrcYuRqCukOMijckted8Z9eg="));  
    acc_cfg->turn_cfg.turn_auth_cred.data.static_cred.realm = pj_str((char*)"");
    acc_cfg->turn_cfg.turn_auth_cred.data.static_cred.nonce = pj_str((char*)"");*/
				...
				if (!info.has_registration) {
					...
					if (m_PresenceStatus == PJRPID_ACTIVITY_BUSY) {
						...
						if (true) {
                            ...
                            } else if (accountSettings.earlyMedia == _T("button") && accountSettings.AA) {
                                str.AppendFormat(_T(" (%s)"), Translate(_T("Early Media")));
                            }

                            if (accountSettings.EM) {
                                str.AppendFormat(_T(" (%s)"), Translate(_T("Early Media")));
                            }
						}
					}
